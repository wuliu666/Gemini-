const API_BASE_URL = "https://wuliu.pythonanywhere.com";
...
async function verifyKey() {
    const p = document.getElementById('secretKey').value.trim();
    if(!p) return;
    try {
        ...
        if(res.ok) {
            ...
            document.getElementById('modelSelect').value = localStorage.getItem('model_type_' + currentUserKey) || 'fast';
            ...
            switchChat(HUB_ID);
        } else { alert(d.error || "验证失败，请联系管理员"); }
    } catch(e) { alert("网络连接失败，请检查您的网络"); }
}
...
function logout() { openConfirmModal(() => { localStorage.removeItem('user_secret_key'); location.reload(); }); }
...
async function saveApiSettings() { alert("配置已保存"); closeApiModal(); }

// 后台面板
async function openAdminPanel() { document.getElementById('adminModal').classList.add('show'); await refreshKeyList(); }
...
async function refreshKeyList() {
    ...
    try {
        ...
        if(res.ok) {
            const d = await res.json();
            for(let k in d.keys) {
                if(k === ak) continue; const info = d.keys[k]; const tr = document.createElement('tr');
                if(info.is_deleted) tr.className = 'status-del';
                tr.innerHTML = `<td style="padding: 10px; border-bottom: 1px solid var(--border-color);"><div style="display: flex; align-items: center; justify-content: space-between; font-family: monospace; font-size: 0.9rem;">${k}<button class="action-btn" style="margin: 0; padding: 2px 6px; font-size: 0.8rem;" onclick="copyAdminKey('${k}', this)" title="复制密钥">📋</button></div></td><td style="padding: 10px; border-bottom: 1px solid var(--border-color);">${info.note}</td><td style="padding: 10px; border-bottom: 1px solid var(--border-color);"><button class="modal-btn" style="padding:4px 8px; font-size:12px; background:${info.is_deleted?'#34c759':'#ff3b30'}; color:white;" onclick="toggleKeyStatus('${k}')">${info.is_deleted?'恢复':'停用'}</button></td>`;
                tb.appendChild(tr);
            }
        }
    } catch(e) { console.log("加载密钥列表失败", e); }
}
async function toggleKeyStatus(t) { const ak = localStorage.getItem('user_secret_key'); await fetch(`${API_BASE_URL}/admin/toggle_delete`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({admin_key:ak, target_key:t}) }); await refreshKeyList(); }
async function generateNewKey() { const ak = localStorage.getItem('user_secret_key'); const n = document.getElementById('newKeyNote').value.trim(); if(!n) return alert("请输入备注信息"); await fetch(`${API_BASE_URL}/admin/create`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({admin_key:ak, note:n}) }); document.getElementById('newKeyNote').value = ''; await refreshKeyList(); }
...
function renderAssetLibraryTool(mode) {
    ...
    <div style="max-width: 1000px; margin: 0 auto; width: 100%; padding: 30px; box-sizing: border-box; animation: pop 0.3s ease;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; border-bottom: 1px solid var(--border-color); padding-bottom: 16px;">
            <div><h2 style="margin: 0 0 6px 0;">${titleText}</h2><div style="font-size: 0.85rem; color: var(--text-secondary);">${descText}</div></div>`;
    
    html += `<div style="display:flex; gap:10px;">`;
    if(!isBulkMode) {
        html += `<button onclick="toggleBulkMode()" style="background: var(--bg-input); color: var(--text-main); border: 1px solid var(--border-color); padding: 10px 20px; border-radius: 8px; cursor: pointer; font-weight: 600;" title="多选并批量下载/分类/删除">☑️ 批量操作</button>`;
    }
    if (canUpload && !isBulkMode) {
        html += `<button onclick="document.getElementById('batchAssetUpload').click()" style="background: var(--bg-user-msg); color: white; border: none; padding: 10px 20px; border-radius: 8px; cursor: pointer; font-weight: 600;" title="支持多选图片进行批量导入">＋ 添加新素材</button>`;
    }
    html += `</div></div><div style="display: flex; gap: 10px; margin-bottom: 24px;">
           <button class="nav-btn ${currentAssetFilter === 'all' ? 'active' : ''}" style="padding: 8px 16px;" onclick="filterAssets('all')">全部展示</button>
           <button class="nav-btn ${currentAssetFilter === 'character' ? 'active' : ''}" style="padding: 8px 16px;" onclick="filterAssets('character')">👤 角色设定</button>
           <button class="nav-btn ${currentAssetFilter === 'scene' ? 'active' : ''}" style="padding: 8px 16px;" onclick="filterAssets('scene')">🏞️ 场景概念</button>
        </div><div id="assetGrid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 20px;"></div></div>`;
    ...
    if (isBulkMode) {
        ...
        // 权限：成员在团队库只能下载。管理员和个人库拥有分类/删除权限
    ...
    for (let file of files) {
        await new Promise((resolve) => {
            ...
            reader.onload = (e) => {
                ...
                if (currentLibraryMode === 'team') teamAssets.unshift(newAsset);
                else personalAssets.unshift(newAsset);
                resolve();
            };
            reader.readAsDataURL(file);
        });
    }
    if (currentLibraryMode === 'team') localStorage.setItem('team_assets', JSON.stringify(teamAssets));
    else localStorage.setItem('personal_assets_' + currentUserKey, JSON.stringify(personalAssets));
    
    input.value = ''; document.getElementById('chatBox').innerHTML = renderAssetLibraryTool(currentLibraryMode); renderAssetGrid();
}
...
function renderAssetGrid() {
    ...
    filtered.forEach(asset => {
        ...
        // 批量模式下增加蒙层和 Checkbox
        if (isBulkMode) {
            cardHtml += `<div class="bulk-overlay" onclick="toggleSelectAsset('${asset.id}')"></div>`;
            cardHtml += `<div class="checkbox-icon">✓</div>`;
        }

        cardHtml += `
            <img src="${asset.image}" style="width: 100%; height: 240px; object-fit: cover; border-bottom: 1px solid var(--border-color);">
            <div style="padding: 16px;">
                <div style="font-weight: bold; margin-bottom: 6px; font-size: 1.05rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="${asset.title}">${asset.title}</div>
                <div style="font-size: 0.8rem; color: var(--text-secondary); margin-bottom: 12px; display: inline-block; background: var(--bg-container); padding: 4px 8px; border-radius: 6px; border: 1px solid var(--border-color);">${asset.type === 'character' ? '👤 角色设定' : '🏞️ 场景概念'}</div>
                <div style="display: flex; gap: 8px;">
                    <button class="nav-btn" style="flex: 1; padding: 8px; font-size: 0.85rem;" title="复制提示词与Seed参数" onclick="copyAssetPrompt('${asset.id}')">📋 词+Seed</button>
                    <button class="nav-btn" style="flex: 1; padding: 8px; font-size: 0.85rem; border-color: var(--shen-color); color: var(--shen-color);" title="去生图控制台载入此素材" onclick="useAssetInGen('${asset.id}')">🎨 去创作</button>
                </div>`;
        if (canManage && !isBulkMode) {
            cardHtml += `
            <div style="display: flex; gap: 8px; margin-top: 8px;">
                <button class="nav-btn" style="flex: 1; padding: 6px; font-size: 0.85rem;" title="单独编辑素材属性" onclick="editAsset('${asset.id}')">✏️ 编辑</button>
                <button class="nav-btn" style="flex: 1; padding: 6px; font-size: 0.85rem; border: none; color: var(--danger-color); background: transparent; opacity: 0.7;" title="删除此素材" onclick="deleteAsset('${asset.id}')" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.7'">🗑️ 删除</button>
            </div>`;
        }
...
function toggleBulkMode() {
    ...
    document.getElementById('chatBox').innerHTML = renderAssetLibraryTool(currentLibraryMode);
    renderAssetGrid();
}
...
async function executeBulkDownload() {
    ...
    selectedAssetIds.forEach((id) => {
        ...
        if(asset && asset.image) {
            count++;
            const data = asset.image.split(',')[1];
            // 简单推断扩展名
            let ext = 'png';
            if(asset.image.includes('jpeg')) ext = 'jpg';
            else if(asset.image.includes('webp')) ext = 'webp';
            zip.file(`${asset.title}_${count}.${ext}`, data, {base64: true});
        }
    });
    zip.generateAsync({type: "blob"}).then(content => {
        const link = document.createElement('a'); link.href = URL.createObjectURL(content); link.download = `素材批量下载_${Date.now()}.zip`; link.click();
        toggleBulkMode(); // 下载完毕退出批量模式
    });
}
...
function executeBulkDelete() {
    ...
    openConfirmModal(() => {
        ...
        toggleBulkMode(); // 删除完毕后直接退出，里面包含了重新渲染
    });
}

function openBulkCategoryModal() {
    if(selectedAssetIds.size === 0) return alert("请先选择素材！");
    document.getElementById('bulkCategoryModal').classList.add('show');
}
...
function confirmBulkCategory() {
    const newType = document.getElementById('bulkCategorySelect').value;
    const sourceArray = currentLibraryMode === 'team' ? teamAssets : personalAssets;
    
    sourceArray.forEach(asset => {
        if(selectedAssetIds.has(asset.id)) { asset.type = newType; }
    });
...
function editAsset(id) {
    editingAssetId = id; const sourceArray = currentLibraryMode === 'team' ? teamAssets : personalAssets;
    const asset = sourceArray.find(a => a.id === id); if(!asset) return;
    document.getElementById('editAssetTitle').value = asset.title; document.getElementById('editAssetType').value = asset.type; document.getElementById('editAssetPrompt').value = asset.prompt || '';
    document.getElementById('editAssetModal').classList.add('show');
}
function saveAssetEdit() {
    const sourceArray = currentLibraryMode === 'team' ? teamAssets : personalAssets;
    const asset = sourceArray.find(a => a.id === editingAssetId); if(!asset) return;
    asset.title = document.getElementById('editAssetTitle').value.trim(); asset.type = document.getElementById('editAssetType').value; asset.prompt = document.getElementById('editAssetPrompt').value.trim();
...
function deleteAsset(id) {
    openConfirmModal(() => {
        if (currentLibraryMode === 'team') { teamAssets = teamAssets.filter(a => a.id !== id); localStorage.setItem('team_assets', JSON.stringify(teamAssets)); } 
        else { personalAssets = personalAssets.filter(a => a.id !== id); localStorage.setItem('personal_assets_' + currentUserKey, JSON.stringify(personalAssets)); }
...
function useAssetInGen(assetId) { const sourceArray = currentLibraryMode === 'team' ? teamAssets : personalAssets; const asset = sourceArray.find(a => a.id === assetId); if (!asset) return; switchChat(IMAGE_GEN_ID); currentUploadedImageBase64 = asset.image; const wrap = document.getElementById('imgUploadPreview'); wrap.style.display = 'inline-block'; wrap.innerHTML = `<div class="img-preview-wrap"><img src="${currentUploadedImageBase64}" class="img-preview-thumb"><div class="img-preview-close" onclick="clearGenImage()">×</div></div>`; document.getElementById('imgGenInput').value = asset.prompt || ''; }
...
async function runImageSplitter() {
    ...
    zip.generateAsync({ type: 'blob' }).then(function(content) { const link = document.createElement('a'); link.href = URL.createObjectURL(content); link.download = 'processed_images.zip'; link.click(); statusDiv.innerText = '处理成功'; processBtn.disabled = false; });
}
...
function sendImageGenMessage() {
    const input = document.getElementById('imgGenInput'), msg = input.value.trim(), chat = chats.find(c => c.id === IMAGE_GEN_ID), w = document.getElementById('imgWidth').value, h = document.getElementById('imgHeight').value, sel = document.getElementById('imgGenModelSelect'), modelText = sel.options[sel.selectedIndex].text;
    if(!msg && !currentUploadedImageBase64) return;
    document.getElementById('imgGenSettingsPanel').style.display = 'none';
    chat.messages.push({ role: 'user', content: `【模型】${modelText}\n【尺寸设定】${currentSelectedRatioText} (${w}x${h}) | ${currentSelectedResText}\n【提示词】${msg || '（无）'}`, attachedImage: currentUploadedImageBase64 });
...
function downloadGalleryZip(msgIndex) {
    ...
    zip.generateAsync({ type: 'blob' }).then(content => { const link = document.createElement('a'); link.href = URL.createObjectURL(content); link.download = `Images.zip`; link.click(); });
}
...
function switchChat(id) { 
    isBulkMode = false; selectedAssetIds.clear(); // 切换时强制重置批量模式
    currentChatId = id; 
    ...
    inputSec.style.display = 'none'; imgGenSec.style.display = 'none'; backBtn.style.display = 'none'; editIcon.style.display = 'none';
...
function createNewStoryboard() { const id = Date.now().toString(); chats.unshift({id, title:"未命名分镜项目", messages:[], isPinned:false, isFavorite:false, isStoryboard:true}); saveChats(); switchChat(id); }
