...
KEYS_FILE = "/home/wuliu/mysite/keys.json"
...
def get_config():
    ...
    keys = load_keys()
    conf = keys.get('__GLOBAL_CONFIG__', {"api_source": "gemini", "api_key": ""})
    return jsonify(conf)
...
def save_config():
    ...
    keys['__GLOBAL_CONFIG__'] = {
        "api_source": data.get('api_source', 'gemini'),
...
def chat():
    ...
    model_choice = data.get('model_type', 'fast') 
    ...
    dynamic_key = global_conf.get('api_key', '')
    source = global_conf.get('api_source', 'gemini')

    if not dynamic_key:
        return jsonify({"error": "系统暂无全局访问通道，请联系管理员配置~"}), 400

    actual_model_name = MODEL_MAP.get(model_choice, "gemini-1.5-flash-latest")
    ...
    try:
        if source == "gemini":
            ...
            # 🌟 新增 GRSAI 分发逻辑
            api_url = "https://www.geeknow.top/api/v1/chat/completions"
            if source == "grsai":
            ...
            if resp.ok:
                ...
                return jsonify({"error": f"中转API返回错误: {resp.text}"}), 500
